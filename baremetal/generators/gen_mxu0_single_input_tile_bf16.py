#!/usr/bin/env python3
"""
gen_mxu0_single_input_tile_bf16.py — FP8-input single-tile matmul computing C = A @ B.

For the 32×32 case, this is one full MXU0 tile:
  1. DMA loads B (FP8) into bank 4
  2. XLU transposes bank 4 → bank 5
  3. VMATPUSH loads bank 5 (= B^T) into MXU0 weight buffer
...
C = sa_bf16(A_t, W_t, scale_exp=0).numpy().astype(np.float32)
