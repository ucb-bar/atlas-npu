#!/usr/bin/env python3
"""Generate test vectors for `smolvla_elementwise_div.S`.

Port of npu-model/npu_model/configs/programs/smolvla_elementwise_div.py.
No dedicated divide op — synthesizes A/B as vrecip(B) * A. Inputs match
npu-model's kernel (`torch.manual_seed(45); randn(32,32,bf16)`, then B
has |b|<0.25 clamped to 0.5 to avoid vrecip blowup). Golden output
comes from atlas's `VectorEngineModel` (via `run_unary_rows("rcp", ...)`
followed by `run_binary_rows("mul", ...)`), not PyTorch — so the
harness reflects atlas's BF16 rcp/mul semantics exactly as the RTL
executes them.
"""
...
def bf16_tile_to_bank_rows(tile: torch.Tensor) -> list[list[int]]:
    ...
    plug straight into vpu_gen_utils.tensor_preloads / run_*_rows.
    """
...
def main():
    ...
    input_b = torch.where(
        input_b.abs() < 0.25, torch.full_like(input_b, 0.5), input_b
    )
    ...
    inv_b_rows = run_unary_rows("rcp", b_rows)
    out_rows = run_binary_rows("mul", a_rows, inv_b_rows)
