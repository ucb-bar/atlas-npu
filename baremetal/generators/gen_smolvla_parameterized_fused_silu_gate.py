#!/usr/bin/env python3
"""Generate test vectors for `smolvla_parameterized_fused_silu_gate.S` (64x64).

Port of jrmills20 parameterized_fused_silu_gate.py. Despite the "gate" in
the kernel name, this is single-input silu(x) = x * sigmoid(x); there is
no separate gate operand. Kept as a separate file for parity with the
npu-model program namespace; functionally identical to
gen_smolvla_parameterized_silu.py.
"""
