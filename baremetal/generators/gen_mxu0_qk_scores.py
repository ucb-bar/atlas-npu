...
#
# The MXU0 accumulates in BF16 (accbuf), then VMATPOP.FP8 converts
...
# ------------------------------------------------------------------
# Instantiate SA functional model (BF16 output only)
#
# The hardware flow for FP8 intermediates is:
#   VMATMUL → accbuf (BF16) → VMATMUL.ACC → accbuf (BF16) → VMATPOP.FP8
#
# We always accumulate in BF16 and apply the FP8 pop conversion
# separately, matching the hardware pipeline.
# ------------------------------------------------------------------
...
#
# SARTLLinearFunction computes F.linear: y = x @ w^T
...
Q_bf16 = sa_bf16(X_t, WQ_t, scale_exp=0).numpy().astype(np.float32)   # 32 x 64
K_bf16 = sa_bf16(X_t, WK_t, scale_exp=0).numpy().astype(np.float32)   # 32 x 64
...
scores = sa_bf16(Q_t, K_t, scale_exp=0).numpy().astype(np.float32)  # 32 x 32
