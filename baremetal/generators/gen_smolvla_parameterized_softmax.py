#!/usr/bin/env python3
"""Generate test vectors for `smolvla_parameterized_softmax.S` (128x32).

Port of jrmills20 parameterized_softmax.py. N=32 fixed, M=128 (4 groups).
Each 32-row group is processed independently — softmax is local per group
because the rowmax/rowsum only span a single 32-lane row under atlas's
VREDMAX.ROW/VREDSUM.ROW pair-ops.
"""
...
def main():
    ...
    x = (torch.randn(M, N, dtype=torch.bfloat16) * 2.0).to(torch.bfloat16)
