#!/usr/bin/env python3
"""Generate test vectors for `smolvla_elementwise_add.S`.

Port of npu-model/npu_model/configs/programs/smolvla_elementwise_add.py.
Inputs are the same seeded random BF16 tensors npu-model uses
(`torch.manual_seed(42); randn(32,32,bf16)` for A then B). Golden output
comes from atlas's `VectorEngineModel` (via `run_binary_rows("add", ...)`),
not PyTorch — so the harness reflects atlas's BF16 add semantics.
"""
