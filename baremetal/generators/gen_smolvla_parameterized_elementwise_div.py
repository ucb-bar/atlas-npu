#!/usr/bin/env python3
"""Generate test vectors for `smolvla_parameterized_elementwise_div.S` (64x64).

Port of jrmills20 parameterized_elementwise_div.py (npu-model-tapeout-kernels).
Atlas has no dedicated divide — synthesizes A / B as vrecip(B) * A. B is
clamped so |b| >= 0.25 (replacing smaller magnitudes with 0.5) to avoid
vrecip blowup, matching stock gen_smolvla_elementwise_div.
Golden comes from atlas's VectorEngineModel: run_unary_rows("rcp", ...)
followed by run_binary_rows("mul", ...).
"""
