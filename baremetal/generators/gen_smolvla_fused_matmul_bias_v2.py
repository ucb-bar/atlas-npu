#!/usr/bin/env python3
"""Generate test vectors for `smolvla_fused_matmul_bias_v2.S`.
...
quantization. Matches the sketch in
`.claude/npu-model/smolvla_fused_matmul_bias.py` and the existing
`smolvla_matmul.S` activation path. Simpler than V1 but does NOT
...
uint8 for BOTH the DRAM preloads and the SARTL golden input. SARTL
bypasses its internal conversion when handed `torch.uint8`
(`software_models/mxu0_sa/systolic_array_rtl_linear.py:274`), so the
golden matmul sees exactly the bytes the assembly will load.
"""
...
def main():
    ...
    # Raw E4M3 bytes — these same bytes feed both the DRAM preloads and
    # the SARTL golden. SARTL bypasses its internal fp32->E4M3 converter
    # when given torch.uint8, so the golden sees exactly what the DUT
    # will load from DRAM.
    ...
    # SARTL computes y = x @ w^T. For A @ B, pass w = B^T.
    w_bytes = b_bytes.T.contiguous()
    mat_bf16 = sa_bf16(a_bytes, w_bytes, scale_exp=0).to(torch.bfloat16)
