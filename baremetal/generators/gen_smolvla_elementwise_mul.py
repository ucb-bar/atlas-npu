#!/usr/bin/env python3
"""Generate test vectors for `smolvla_elementwise_mul.S`.

Port of npu-model/npu_model/configs/programs/smolvla_elementwise_mul.py.
Inputs match that kernel's seed (`torch.manual_seed(43); randn(32,32,bf16)`
for A then B). Golden output comes from atlas's `VectorEngineModel`
(via `run_binary_rows("mul", ...)`), not PyTorch — so the harness
reflects atlas's BF16 mul semantics.
"""
