...
import atlas.mxu.FPUtils

/** One compute lane in the inner-product-tree MXU.
  *
  * Multiplies an activation vector by one selected weight vector, reduces the
  * products through an anchor-aligned accumulation tree, and optionally adds a
  * partial sum before converting the result back to IEEE format.
  *
  * @param p  IPT geometry and format parameters for this lane.
  */
class AnchorAccumulationTree(p: InnerProductTreeParams) extends Module {
  ...
  val io = IO(new Bundle {
    ...
    val psum       = Input(UInt(p.outputFmt.ieeeWidth.W))
  ...
  val psumExpField = io.psum(oFmt.ieeeWidth - 2, oFmt.mantissaBits)
  val psumFrac = io.psum(oFmt.mantissaBits - 1, 0)
  val psumIsZero = psumExpField === 0.U && psumFrac === 0.U
  val psumUnbExp = psumExpField.zext -& oFmt.ieeeBias.S(expW.W)
  ...
  // ── S2: Anchor-aligned conversion (FPUtils functions) ─────────────
  val prodInt = Wire(Vec(N, SInt(intW.W)))
  for (j <- 0 until N) {
    prodInt(j) := FPUtils.e4m3ProdToAlignedInt(prod_s2(j), anchor_s2, intW, expW)
  }

  val psumAligned = FPUtils.ieeeToAlignedInt(psum_s2, anchor_s2, oFmt, intW, expW)

  val addendInt_s2 = WireDefault(0.S(intW.W))
  when(accumulate_s2) { addendInt_s2 := psumAligned }
  ...
  // ── S4: Back-conversion to IEEE (FPUtils function) ────────────────
  val rawBF16 = FPUtils.alignedIntToIEEE(totalInt_s4, anchor_s4, oFmt, intW, expW)
  val sign = rawBF16(15)
