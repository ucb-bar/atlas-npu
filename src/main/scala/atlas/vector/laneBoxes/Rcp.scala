...
// Input bundles
class RcpReq(wordWidth: Int, numLanes: Int, tagWidth: Int) extends Bundle {
...
// Output bundles
class RcpResp(wordWidth: Int, numLanes: Int, tagWidth: Int) extends Bundle {
    // val laneMask = UInt(numLanes.W)
    val result = Vec(numLanes, UInt(wordWidth.W))
}

class Rcp(BF16T: AtlasFPType, numLanes: Int = 16, tagWidth: Int = 16) extends Module with HasPipelineParams {
    val io = IO(new Bundle {
        val req = Flipped(Valid(new RcpReq(BF16T.wordWidth, numLanes, tagWidth)))
        val resp = Valid(new RcpResp(BF16T.wordWidth, numLanes, tagWidth))
    })
    ...
    val lut = Module(new RcpLUT(numLanes, addrBits, BF16T.lutValM, BF16T.lutValN)) // 16, 7, 1, 12
    ...
    val neg = VecInit(io.req.bits.aVec.map(a => a(BF16T.wordWidth - 1)))
