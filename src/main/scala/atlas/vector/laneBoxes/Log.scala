...
// Input bundles
class LogReq(wordWidth: Int, numLanes: Int, tagWidth: Int) extends Bundle {
    val laneMask = UInt(numLanes.W)
    val aVec = Vec(numLanes, UInt(wordWidth.W))
}

// Output bundles
class LogResp(wordWidth: Int, numLanes: Int, tagWidth: Int) extends Bundle {
    // val laneMask = UInt(numLanes.W)
    val result = Vec(numLanes, UInt(wordWidth.W))
}

class Log(BF16T: AtlasFPType, numLanes: Int = 16, tagWidth: Int = 16) extends Module with HasPipelineParams {
    val io = IO(new Bundle {
        val req = Flipped(Valid(new LogReq(BF16T.wordWidth, numLanes, tagWidth)))
        val resp = Valid(new LogResp(BF16T.wordWidth, numLanes, tagWidth))
    })
    ...
    val lut = Module(new LogLUT(numLanes, addrBits, 9, BF16T.lutValN)) // 16, 7, 9, 12
