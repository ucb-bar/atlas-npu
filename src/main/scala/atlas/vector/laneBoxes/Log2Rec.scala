...
// Output bundles
class Log2Resp(wordWidth: Int, numLanes: Int, tagWidth: Int) extends Bundle {
    val tag = UInt(tagWidth.W)
    val whichBank = UInt(5.W)
    val wRow= UInt(7.W)
    val laneMask = UInt(numLanes.W)
...
class Log2Rec(BF16T: AtlasFPType, numLanes: Int = 16, tagWidth: Int = 16) extends Module with HasPipelineParams {
    ...
    val io = IO(new Bundle {
        val req = Flipped(Decoupled(new Log2Req(BF16T.wordWidth, numLanes, tagWidth)))
        val resp = Decoupled(new Log2Resp(BF16T.wordWidth, numLanes, tagWidth))
    })
  
    val laneEnable = VecInit((io.req.bits.laneMask & VecInit.fill(numLanes)(io.req.fire).asUInt).asBools)
    val lut = Module(new LogLUT(ports = numLanes, addrBits = BF16T.lutAddrBits, m = 1, n = n))
    val log2Modules = Seq.fill(numLanes) { Module(new Log2(BF16T)) }
    ...
    for (i <- 0 until numLanes) {
        log2Modules(i).io.x := io.req.bits.xVec(i)
        log2Modules(i).io.valid := laneEnable(i)

        lut.io.raddr(i) := log2Modules(i).io.lutAddr
        lut.io.ren(i) := log2Modules(i).io.lutEn

        log2Modules(i).io.lutVal1 := lut.io.rdata(0)(i)
        log2Modules(i).io.lutVal2 := lut.io.rdata(1)(i)
    }
    ...
    for (i <-0 until numLanes) {
        io.resp.bits.result(i) := log2Modules(i).io.result
    }
}