...
class MulRec(BF16T: AtlasFPType, numLanes: Int = 16, tagWidth: Int = 16) extends Module with HasPipelineParams {
    val io = IO(new Bundle {
        val req = Flipped(Valid(new MulReq(BF16T.wordWidth, numLanes, tagWidth)))
        val resp = Valid(new MulResp(BF16T.wordWidth, numLanes, tagWidth))
    })
    ...
    multipliers.zipWithIndex.foreach { case (mul, i) =>
        mul.io.a := aRecVecRaw(i)
        mul.io.b := bRecVecRaw(i)
        invalidExc(i) := mul.io.invalidExc
        productRec(i) := mul.io.rawOut
    }

    // Stage 1: Raw -> RecFN -> FN
    val productRecNext = maskLaneNext(productRec, laneEnable, bp = bp(1))
