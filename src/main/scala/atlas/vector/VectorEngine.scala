...
//   when the engine is ready (`io.busy` low).
...
class VectorEngine(val p: VpuParams) extends Module  {
  ...
  val isRMax2    = (fsm.io.out.inst2 === VPUOp.rmax    )
  val isRMin2    = (fsm.io.out.inst2 === VPUOp.rmin    )
  ...
  val isRSum2    = (fsm.io.out.inst2 === VPUOp.rsum    )
  ...
  val isRMaxValid    = (isRMax1    && io.data1.valid) || (isRMax2    && io.data2.valid)
  val isRMinValid    = (isRMin1    && io.data1.valid) || (isRMin2    && io.data2.valid)
  ...
  val isRSumValid    = (isRSum1    && io.data1.valid) || (isRSum2    && io.data2.valid)
  ...
  // ── AddSub / RowSum (csum handled separately by ColAddVec) ──
  val addSubSum = Module(new AddSubSumVec(p.BF16))
  addSubSum.io.req.valid                     := (isAddValid || isSubValid || isRSumValid)
  addSubSum.io.req.bits.isDoneReadingColSum   := fsm.io.out.isDoneReadingColSum
  addSubSum.io.req.bits.isSub                := isSubValid
  addSubSum.io.req.bits.isSum                := isRSumValid
  addSubSum.io.req.bits.aVec                 := Mux(isAddValid || isSubValid || isRSum1, vector1, vector2)
  ...
  val rowMax = Module(new RowMax(p.BF16))
  rowMax.io.req.valid    := isRMaxValid
  rowMax.io.req.bits.aVec := Mux(isRMax1, vector1, vector2)

  val rowMin = Module(new RowMin(p.BF16))
  rowMin.io.req.valid    := isRMinValid
  rowMin.io.req.bits.aVec := Mux(isRMin1, vector1, vector2)
  ...
  dataInFire1 := MuxCase(false.B, Seq(
    ((isAdd1 || isSub1 || isRSum1)           && io.data1.valid) -> addSubSum.io.req.valid,
    ...
    (isRMax1                                 && io.data1.valid) -> rowMax.io.req.valid,
    (isRMin1                                 && io.data1.valid) -> rowMin.io.req.valid,
  ...
  dataInFire2 := MuxCase(false.B, Seq(
    ((isAdd2 || isSub2 || isRSum2)           && io.data2.valid) -> addSubSum.io.req.valid,
    ...
    (isRMax2                                 && io.data2.valid) -> rowMax.io.req.valid,
    (isRMin2                                 && io.data2.valid) -> rowMin.io.req.valid,
  ...
  dataOutFire1 := MuxCase(false.B, Seq(
    (isAdd1 || isSub1 || isRSum1) -> addSubSum.io.resp.valid,
    ...
    isRMax1                       -> rowMax.io.resp.valid,
    isRMin1                       -> rowMin.io.resp.valid,
  ...
  dataOutFire2 := MuxCase(false.B, Seq(
    ...
    isRSum2                -> addSubSum.io.resp.valid,
    ...
    isRMax2                -> rowMax.io.resp.valid,
    isRMin2                -> rowMin.io.resp.valid,
  ...
  writeData1 := MuxCase(0.U, Seq(
    (isAdd1 || isSub1 || isRSum1) -> addSubSum.io.resp.bits.result.asUInt,
    ...
    isRMax1                       -> rowMax.io.resp.bits.result.asUInt,
    isRMin1                       -> rowMin.io.resp.bits.result.asUInt,
  ...
  writeData2 := MuxCase(0.U, Seq(
    (isAdd2 || isSub2 || isRSum2) -> addSubSum.io.resp.bits.result.asUInt,
    ...
    isRMax2                       -> rowMax.io.resp.bits.result.asUInt,
    isRMin2                       -> rowMin.io.resp.bits.result.asUInt,
