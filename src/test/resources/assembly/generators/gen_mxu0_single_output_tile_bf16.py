#!/usr/bin/env python3
"""
gen_mxu0_single_output_tile_bf16.py
...
Conventions:
  ...
  - VMATPUSH.ACC.FP8.MXU0 initializes the MXU0 accumulator from that FP8 tile.
  - Final output is BF16 popped from the accumulator.

Reference model:
  - Uses SARTLLinearFunction (RTL-accurate MXU0 software model) for the
    matmul accumulation with optional FP8 bias.
  - SARTLLinearFunction computes F.linear: y = x @ w^T + b
  - For C = A @ B + bias, pass w = B^T so y = A @ B^T^T = A @ B.

Usage:
    python3 gen_mxu0_single_output_tile_bf16.py 42 1
"""
...
def main():
    ...
    # ------------------------------------------------------------------
    # Reference computation via SA functional model
    #
    # SARTLLinearFunction computes F.linear: y = x @ w^T + b
    # We want C = A @ B + bias, so pass w = B^T → y = A @ B^T^T = A @ B.
    #
    # Bias handling: SARTLLinearFunction adds the E4M3 bias at k_tile 0
    ...
    C_ref = sa_bf16(A_t, W_t, b_q=b_t, scale_exp=0).numpy().astype(np.float32)
    ...
    sys.stderr.write(
        f"[gen_mxu0_single_output_tile_bf16] "
