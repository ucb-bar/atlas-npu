#!/usr/bin/env python3
"""
gen_mxu_single_output_tile_bf16.py
...
Conventions:
  ...
  - VMATPUSH.ACC.FP8.MXU0 initializes the MXU accumulator from that FP8 tile.
  - Final output is BF16 popped from the accumulator.

Reference model used here:
  - Inputs A, B, bias are quantized to FP8 first.
  - Bias preload initializes the accumulator from FP8.
  - Each K-tile update is computed in float32, then quantized to BF16
    before being used as the next tile's addend (matching old script semantics).
  - Final output is BF16.

Usage:
    python3 gen_mxu_single_output_tile_bf16.py 42 1
"""
...
def main():
    ...
    # Build accumulator reference matching old script semantics
    # Accumulator semantics for expected values here:
    #   acc <- dequantize(fp8_bias_tile)
    #   acc <- bf16(acc + partial) for each K tile
    #   final output <- bf16(acc)
    acc = quantize_bf16(
        np.tile(bias_q.reshape(1, TILE), (TILE, 1))
    ).astype(np.float32)

    # Build DRAM payloads for A tiles and logical B tiles
    # IMPORTANT:
    #   We store B in logical orientation.
    #   Assembly XLU-transposes each B tile before VMATPUSH.W.MXU0.
    act_beats = []
    wgt_beats = []
    ...
    for t in range(num_k_tiles):
        ...
        # Reference partial accumulation with BF16 writeback each tile
        partial = a_tile @ b_tile
        acc = quantize_bf16(acc + partial).astype(np.float32)

        # DRAM payloads
        act_beats.extend(pack_fp8_tile(a_tile))
        wgt_beats.extend(pack_fp8_tile(b_tile))

    C_ref = quantize_bf16(acc)

    # Bias tile for accumulator preload
    # bias_bank[r, j] = bias[j] for all rows r
    # Stored as FP8 in DRAM; hardware upcasts on VMATPUSH.ACC.FP8.MXU0.
    if use_bias:
