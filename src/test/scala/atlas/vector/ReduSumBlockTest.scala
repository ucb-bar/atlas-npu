...
object PersistentVcsReduSumBlockSimulator extends Simulator[VcsBackend] with PeekPokeAPI {

  private val test_name = "ReduSumBlockTest"
...
class ReduSumBlockTest extends AnyFlatSpec with Matchers with PeekPokeAPI {
  ...
  it should "compute the masked sum and report relative error" in {
    PersistentVcsReduSumBlockSimulator.simulate(new ReduSumRec(fptype, numLanes, tagWidth)) { module =>
