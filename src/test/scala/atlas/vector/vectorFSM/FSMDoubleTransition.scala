...
object PersistentVcsFSMDoubleTransitionSimulator extends Simulator[VcsBackend] with PeekPokeAPI {

  private val test_name = "FSMDoubleTransition"
...
class FSMDoubleTransition extends AnyFlatSpec with Matchers with PeekPokeAPI {
    ...
    // it should "Verify the transition from double to double" in {
    ...
    it should "Verify the transition from double to single" in {
        PersistentVcsFSMDoubleTransitionSimulator.simulate(new VectorFSM(p)) { module =>
            ...
            for (i <- 0 to 135) {
                ...
                // Input fires at the 65th cycle again which is when the first instruction is done
                    ...
                    dut.io.in.instFire.poke(true.B)
                    dut.io.in.instType.poke(VPUOp.sin)
                    dut.io.in.instReadBank1.poke(11.U)
                    dut.io.in.instReadBank2.poke(12.U)
                    dut.io.in.instWriteBank.poke(13.U)
                }
