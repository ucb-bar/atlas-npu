...
object PersistentVcsFSMOverlapTracking2Simulator extends Simulator[VcsBackend] with PeekPokeAPI {

  private val testName = "FSMOverlapTracking2"
...
class FSMOverlapTracking2 extends AnyFlatSpec with Matchers with PeekPokeAPI {

  //----------- CI/CD INCLUDE --------------
  override def withFixture(test: NoArgTest): Outcome = {
    val outcome = super.withFixture(test)
    if (outcome.isFailed) {
      println("FSMOverlapTracking2=FAILED")
    } else if (outcome.isSucceeded) {
      println("FSMOverlapTracking2=PASSED")
    }
    outcome
  }
  //----------- CI/CD INCLUDE --------------
  ...
  it should "use both physical banks in lockstep for BF16 row reductions" in {
    PersistentVcsFSMOverlapTracking2Simulator.simulate(new VectorFSM(p)) { module =>
