...
object PersistentVcsFSMOverlapTracking4Simulator extends Simulator[VcsBackend] with PeekPokeAPI {

  private val testName = "FSMOverlapTracking4"
...
class FSMOverlapTracking4 extends AnyFlatSpec with Matchers with PeekPokeAPI {

  //----------- CI/CD INCLUDE --------------
  override def withFixture(test: NoArgTest): Outcome = {
    val outcome = super.withFixture(test)
    if (outcome.isFailed) {
      println("FSMOverlapTracking4=FAILED")
    } else if (outcome.isSucceeded) {
      println("FSMOverlapTracking4=PASSED")
    }
    outcome
  }
  //----------- CI/CD INCLUDE --------------
  ...
  it should "block overlap for commands that share a functional unit" in {
    PersistentVcsFSMOverlapTracking4Simulator.simulate(new VectorFSM(p)) { module =>
