...
object PersistentVcsFSMSingleSimulator extends Simulator[VcsBackend] with PeekPokeAPI {

  private val test_name = "FSMSingle"
...
class FSMSingle extends AnyFlatSpec with Matchers with PeekPokeAPI {

    //----------- CI/CD INCLUDE --------------
    override def withFixture(test: NoArgTest): Outcome = {
        val outcome = super.withFixture(test)
        if (outcome.isFailed) {
        println("FSMSingle=FAILED")
        } else if (outcome.isSucceeded) {
        println("FSMSingle=PASSED")
        }
        outcome
    }
    //----------- CI/CD INCLUDE --------------
    ...
    // it should "Verify 1 single input instruction to SINGLE" in {
    ...
    // }
    it should "Verify 1 single input instruction to SINGLE" in {
        PersistentVcsFSMSingleSimulator.simulate(new VectorFSM(p)) { module =>
