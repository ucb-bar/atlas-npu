...
object PersistentVcsLogLUTSimulator extends Simulator[VcsBackend] with PeekPokeAPI {

  private val test_name = "LogLUTTest"
...
class LogLUTTest extends AnyFlatSpec with HasSinCosParams with Matchers with PeekPokeAPI {
  ...
  it should "Print values of LogLUT ranging from 0.5 to 1.0" in {
    ...
    PersistentVcsLogLUTSimulator.simulate(new LogLUT(16, 7, 9, 12)) { module =>
