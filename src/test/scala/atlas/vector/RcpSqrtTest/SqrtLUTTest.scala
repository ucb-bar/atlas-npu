...
object PersistentVcsSqrtLUTSimulator extends Simulator[VcsBackend] with PeekPokeAPI {

  private val test_name = "SqrtLUTTest"
...
class SqrtLUTTest extends AnyFlatSpec with HasSinCosParams with Matchers with PeekPokeAPI {
  it should "Print values of SqrtLUT ranging from 1.0 to 2.0" in {
    ...
    PersistentVcsSqrtLUTSimulator.simulate(new SqrtLUT(16, 7, 9, 12)) { module =>
      ...
      for (i <- 0 until 8) {
        for (k <- 0 until 16) {
          ...
          dut.io.oddExp(k).poke(isOddExp.asBool) // odd exponent to test shifting
        }
