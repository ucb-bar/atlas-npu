...
object PersistentVcsPairwiseMinSimulator extends Simulator[VcsBackend] with PeekPokeAPI {

  private val test_name = "PairwiseMinTest"
...
class PairwiseMinTest extends AnyFlatSpec with HasSinCosParams with Matchers with PeekPokeAPI {
  it should "Verify function of PairwiseMin" in {
    PersistentVcsPairwiseMinSimulator.simulate(new PairWiseMin(AtlasFPType.BF16)) { module =>
      ...
      println("The following are PairwiseMin")
      for (i <- 0 until 10) {
        ...
        val expectedOutput = goldenPairwiseMinVector(inputData1, inputData2)
